...
3. `createdb gravy`
4. `cat data/dailyprices/sql/create_tables.sql | psql gravy`
5. `go run cmd/data/dailyprices/pipeline/main.go` (takes > 5 hours)
6. `cat data/dailyprices/sql/create_indexes.sql | psql gravy`
...
Create a TimescaleDB database called `gravy_timescale_output` (TODO: Add instructions.)

You'll also need to generate the asset IDs and pair stats tables, see `data/assetids` and `data/pairstats` for instructions.
